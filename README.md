...
- 🏗️ 교내 클라우드 플랫폼 **`Pickle`** 은 제안부터 개발과 운영까지 맡고 있고, SW프로젝트관리시스템 **`Opus`** 는 기획과 인프라를 담당했습니다.
- ⚡ SW마에스트로 16기에서 **Knative 기반 scale-to-zero 배포 플랫폼 `LaunchPad`** 를 만들었습니다.
- 🛡️ **BoB 8기 취약점 분석 트랙**을 수료하고, **공군사관학교**에서 망분리 환경 관제와 엔드포인트 보안, 침해사고 대응을 맡았습니다.
- 🔎 글로벌 게임사 **버그바운티**를 비롯해 국립대학교 통합포털 등에 취약점을 제보해 왔습니다.
- 🧩 **LG CNS** 인턴으로 접근 통제와 절차가 갖춰진 엔터프라이즈 환경에서 개발과 배포를 경험했습니다.
...
구성원이 필요한 자원을 신청하면 승인, 자동 생성, 회수로 이어지는 운영 흐름을 설계했습니다.
Proxmox VE 기반 VM 프로비저닝에서 시작해 LLM API 게이트웨이와 GPU 컨테이너까지 제공 리소스를 확장하고 있습니다.
SSH 게이트웨이, 웹 터미널, 도메인 HTTPS 공개, 포트 포워딩 등 접근과 네트워크 기능을 구현했습니다.
지금은 교내 실서비스 오픈을 준비하고 있습니다.
...
교내 해커톤에서 시작해 팀을 모았고, 기획과 인프라를 맡았습니다.
시스템을 실제로 쓰게 될 학과 조교, 사업단 연구원과 이야기하면서 필요한 기능을 정리하고
무엇부터 만들지 순서를 잡았습니다. 지금도 학부에서 실제로 쓰이고 있습니다.
...
SW마에스트로 16기 프로젝트. 코드를 올리면 이미지 빌드부터 실행까지 자동으로 이어지는 배포 플랫폼입니다.
...
| 기간 | 소속 | 역할 |
|:---|:---|:---|
| 2026.07 ~ | Codyssey AI 올인원 2기 | 본과정 참여 중 |
| 2026.01 ~ 2026.02 | SW마에스트로 글로벌 AI·SW역량 강화 교육 | 미국 해외연수 |
| 2025.04 ~ 2025.12 | SW마에스트로 16기 | LaunchPad 설계 및 개발 |
| 2025.03 ~ | 부산대학교 동아리 PNUops | Pickle 제안, 개발, 운영 · Opus 기획 및 인프라 |
| 2025.01 ~ 2025.02 | LG CNS 스마트팩토리 사업부 | 인턴 · RMS 개발 |
